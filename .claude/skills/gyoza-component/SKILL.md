...
指定されたReactコンポーネントファイルをGyoza UIライブラリに登録する。以下の手順を順に実行すること。
...
- `React.forwardRef` でref転送、`.displayName` を設定
- Fumadocs MDXでドキュメント管理
- 日本語サイト

## 手順

### Step 1: コンポーネントファイルの確認と配置
...
4. コンポーネントが以下のGyoza UIパターンに従っているか確認し、必要に応じて調整する：
...
`content/docs/components/<component-name>.mdx` を作成する。以下のテンプレートに従う：
...
import { <ExportedComponents> } from "@/registry/ui/<component-name>"
...
\```tsx
import { <ExportedComponents> } from "@/components/ui/<component-name>"
\```
...
### Step 4: meta.json の更新

`content/docs/meta.json` の `pages` 配列に `"components/<component-name>"` を追加する。
...
を実行して `public/r/<component-name>.json` を生成する。
...
- `content/docs/components/<component-name>.mdx` が作成されている
- `content/docs/meta.json` に追加されている
- `public/r/<component-name>.json` が生成されている
